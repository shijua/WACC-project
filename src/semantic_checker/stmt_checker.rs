...
use crate::semantic_checker::util::{
    expr_to_type, lvalue_to_type, rvalue_to_type, type_check, type_check_array_elem, Error,
};
...
pub fn declaration_check<'a, T>(
    type_given: &Spanned<Type>,
    ident: &'a Spanned<String>,
        ...
        && type_check_array_elem(type_given, &rvalue_type).is_err()
    {
        return Err(Error::new_error(
            ...
            format!(
                ...
                from_span(type_given),
                from_span(&rvalue_type)
            ),
        ));
    }
...
pub fn assignment_check<T>(
    ...
    if lvalue_type == &Type::Any && rvalue_type == &Type::Any {
        return Err(Error::new_error(
            get_span(span),
            "type mismatch in assignment(both side is Type Any)".to_string(),
        ));
    }

    // check if lvalue and rvalue have the same type
    if lvalue_type == &Type::Any
        || lvalue_type == rvalue_type
        || type_check_array_elem(&lvalue_span, &rvalue_span).is_ok()
...
pub fn read_check<T>(
    ...
    if from_span(&lvalue_type) != &Type::IntType && from_span(&lvalue_type) != &Type::CharType {
        return Err(Error::new_error(
            get_span(span),
            format!(
                "type mismatch in read: Expected Int type and Char type, found {:?}.",
                from_span(&lvalue_type)
            ),
        ));
    }
...
pub fn free_check<T>(
    ...
    match from_span(&expr_type) {
        ...
        _ => Err(Error::new_error(
            get_span(&span),
            format!(
                "Type mismatch in free: expected Array or Pair, found {:?}",
                from_span(&expr_type)
            ),
        )),
    }
...
pub fn return_check<T>(
    ...
    if symbol_table.is_func == false {
        return Err(Error::new_error(
            get_span(span),
            "cannot return in the main function".to_string(),
        ));
    }
    ...
    let func_type = from_span(function_table.get(symbol_table.func_name.unwrap()).unwrap())
        .return_type
        .clone();
...
pub fn exit_check<T>(
    ...
    if from_span(&expr_type) != &Type::IntType {
        return Err(Error::new_error(
            get_span(&span),
            format!(
                "Type mismatch in exit: Expected Int type, found {:?}.",
                from_span(&expr_type)
            ),
        ));
    }
...
pub fn if_check(
    expr: &Spanned<Expr>,
    stmt1: &Spanned<ReturningStmt>,
    stmt2: &Spanned<ReturningStmt>,
    ...
    if from_span(&expr_type) != &Type::BoolType {
        return Err(Error::new_error(
            get_span(&expr_type),
            format!(
                "Type mismatch in if: Expected Bool type, found {:?}.",
                from_span(&expr_type)
            ),
        ));
    }
...
pub fn while_check(
    ...
    if from_span(&expr_type) != &Type::BoolType {
        return Err(Error::new_error(
            get_span(&expr_type),
            format!(
                "Type mismatch in while: Expected Bool type, found {:?}.",
                from_span(&expr_type)
            ),
        ));
    }
...
pub fn scope_check(
    ...
    let mut new_symbol_table = SymbolTable::create(
        Some(Box::from(symbol_table)),
        symbol_table.is_func,
        symbol_table.func_name.clone(),
    );
    stmt_check(stmt, &mut new_symbol_table, function_table)
}

pub fn stmt_check<'a>(
    ret_stmt: &'a Spanned<ReturningStmt>,
    symbol_table: &mut SymbolTable<'a>,
    function_table: &HashMap<String, Spanned<Function>>,
) -> Result<Spanned<Type>, Error> {
    ...
    match from_span(stmt_type) {
        ...
        Stmt::Declare(type_given, ident, rvalue) => declaration_check(
            &type_given,
            &ident,
            &rvalue,
            symbol_table,
        ...
        Stmt::Read(lvalue) => read_check(lvalue, symbol_table, ret_stmt),
        Stmt::Free(expr) => free_check(expr, symbol_table, ret_stmt),
        Stmt::Return(expr) => return_check(expr, symbol_table, function_table, ret_stmt),
        Stmt::Exit(expr) => exit_check(expr, symbol_table, ret_stmt),
        Stmt::Print(expr) | Stmt::Println(expr) => print_println_check(expr, symbol_table),
        Stmt::If(expr, stmt1, stmt2) => {
            if_check(expr, &stmt1, &stmt2, symbol_table, function_table)
        }
        Stmt::While(expr, stmt) => while_check(expr, &stmt, symbol_table, function_table),
        Stmt::Scope(stmt) => scope_check(&stmt, symbol_table, function_table),
...
mod stmt_checker_tests {
    ...
    use crate::ast::{Expr, Function, Lvalue, ReturningStmt, Rvalue, Stmt, Type};
    use crate::semantic_checker::stmt_checker::{
        assignment_check, declaration_check, exit_check, free_check, if_check, print_println_check,
        read_check, return_check, scope_check, while_check,
    };
    use crate::symbol_table::SymbolTable;
    ...
    fn declaration_check_test() {
        ...
        assert!(matches!(
            declaration_check(
                ...
                &f,
                &create_span(
                    Rvalue::RExpr(Box::from(create_span(IntLiter(1), empty_span()))),
                    empty_span()
                ),
                &mut symbol_table,
                &function_table,
    ...
    fn assignment_check_test() {
        ...
        assert!(matches!(
            assignment_check(
                &create_span(
                    Lvalue::LIdent(create_span("a".to_string(), empty_span())),
                    empty_span()
                ),
                &create_span(
                    Rvalue::RExpr(Box::from(create_span(CharLiter('a'), empty_span()))),
                    empty_span()
                ),
                &symbol_table,
                &function_table,
    ...
    fn read_check_test() {
        ...
        assert!(matches!(
            read_check(
                &create_span(
                    Lvalue::LIdent(create_span("b".to_string(), empty_span())),
                    empty_span()
                ),
                &symbol_table,
        ...
        assert!(matches!(
            read_check(
                &create_span(
                    Lvalue::LIdent(create_span("c".to_string(), empty_span())),
                    empty_span()
                ),
                &symbol_table,
                &create_span(Type::IntType, empty_span())
    ...
    fn free_check_test() {
        ...
        assert!(matches!(
            free_check(
                &create_span(Expr::Ident("b".to_string()), empty_span()),
                &symbol_table,
                &create_span(
                    Type::Array(Box::new(create_span(Type::IntType, empty_span()))),
                    empty_span()
        ...
        assert!(matches!(
            free_check(
                &create_span(Expr::Ident("c".to_string()), empty_span()),
                &symbol_table,
                &create_span(
                    Type::Pair(
                        Box::new(create_span(Type::IntType, empty_span())),
        ...
        assert!(matches!(
            free_check(
                &create_span(Expr::Ident("a".to_string()), empty_span()),
                &symbol_table,
                &create_span(Type::IntType, empty_span())
            ),
            Err(_)
    ...
    fn return_check_test() {
        ...
        let func = Function {
            ...
            body: (
                ReturningStmt {
                    statement: (
                        Stmt::Return((Expr::IntLiter(0), empty_span())),
                        empty_span(),
                    ),
                    returning: true,
                },
