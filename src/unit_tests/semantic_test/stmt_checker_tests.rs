...
mod stmt_checker_tests {
    use crate::ast::{BinaryOperator, Expr, Lvalue, Rvalue, Stmt, Type, UnaryOperator};
    ...
    fn declaration_check_test() {
        ...
        assert!(matches!(
            declaration_check(&Type::IntType, "a", &Rvalue::RExpr(IntLiter(1)), &mut symbol_table),
            Err(_)
        ));
    }
    ...
    fn assignment_check_test() {
        let mut symbol_table = create_symbol_table();
        assert!(matches!(
            assignment_check(&Lvalue::LIdent("a".to_string()), &Rvalue::RExpr(IntLiter(1)), &symbol_table),
            Ok(Type::IntType)
        ));
        assert!(matches!(
            assignment_check(&Lvalue::LIdent("a".to_string()), &Rvalue::RExpr(Expr::CharLiter('a')), &symbol_table),
            Err(_)
        ));
        assert!(matches!(
            assignment_check(&Lvalue::LIdent("f".to_string()), &Rvalue::RExpr(IntLiter(1)), &symbol_table),
            Err(_)
        ));
    }
    ...
    fn if_check_test() {
        let mut symbol_table = create_symbol_table();
        assert!(matches!(
            if_check(&Expr::Ident("e".to_string()), &Stmt::Skip, &Stmt::Skip, &mut symbol_table),
            Ok(_)
        ));
        assert!(matches!(
            if_check(&Expr::Ident("a".to_string()), &Stmt::Skip, &Stmt::Skip, &mut symbol_table),
            Err(_)
        ));
        // testing for invalid statements
        assert!(matches!(
            if_check(&Expr::Ident("e".to_string()), &Stmt::Read(Lvalue::LIdent("c".to_string())), &Stmt::Skip, &mut symbol_table),
            Err(_)
        ));
        assert!(matches!(
            if_check(&Expr::Ident("e".to_string()), &Stmt::Skip, &Stmt::Read(Lvalue::LIdent("c".to_string())), &mut symbol_table),
            Err(_)
        ));
    }
    ...
    fn while_check_test() {
        let mut symbol_table = create_symbol_table();
        assert!(matches!(
            while_check(&Expr::Ident("e".to_string()), &Stmt::Skip, &mut symbol_table),
            Ok(_)
        ));
        assert!(matches!(
            while_check(&Expr::Ident("a".to_string()), &Stmt::Skip, &mut symbol_table),
            Err(_)
        ));
        // testing for invalid statements
        assert!(matches!(
            while_check(&Expr::Ident("e".to_string()), &Stmt::Read(Lvalue::LIdent("c".to_string())), &mut symbol_table),
            Err(_)
        ));
    }
    ...
    fn scope_check_test() {
        let mut symbol_table = create_symbol_table();
        assert!(matches!(
            scope_check(&Stmt::Skip, &mut symbol_table),
            Ok(_)
        ));
    }
    ...
    fn stmt_check_test() {
        let mut symbol_table = create_symbol_table();
        assert!(matches!(
            stmt_check(&Stmt::Skip, &mut symbol_table),
            Ok(_)
        ));

        assert!(matches!(
            stmt_check(&Stmt::Declare(Type::IntType, "f".to_string(), Rvalue::RExpr(IntLiter(1))), &mut symbol_table),
            Ok(_)
        ));
    }
